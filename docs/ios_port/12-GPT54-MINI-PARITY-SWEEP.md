# GPT-5.4 Mini iOS Parity Sweep Handoff

## Mission

You are taking over the iOS port for Kessho.
...
- Prefer changing files under:
  - `KesshoiOS/Kessho/Audio/`
  - `KesshoiOS/Kessho/State/`
  - `plugins/kessho-background-audio-spike/ios/Sources/KesshoBackgroundAudio/`
...
- Native state decoding now normalizes more web-style payloads before decoding:
  - `KesshoiOS/Kessho/State/SliderState.swift`
- Native preset loading/import now goes through the same normalization path:
  - `KesshoiOS/Kessho/State/PresetManager.swift`
- Capacitor/native background-audio payload decoding now uses the same normalization path:
  - `plugins/kessho-background-audio-spike/ios/Sources/KesshoBackgroundAudio/KesshoBackgroundAudioPlugin.swift`
- Native mixer debug taps are now `#if DEBUG` only:
  - `KesshoiOS/Kessho/Audio/AudioEngine.swift`
...
- Reverb is now much closer to the web graph:
  - the custom FDN in `KesshoiOS/Kessho/Audio/ReverbProcessor.swift` is the default live path
  - Apple reverb is still present only as the explicit `lite` fallback
  - `KesshoiOS/Kessho/Audio/AudioEngine.swift` now uses dedicated per-source reverb send mixers instead of dead send math
...
- The next agent should verify this path audibly and structurally, not replace it again unless a concrete bug is found.
...
- iOS direct source-to-reverb wiring:
  - `KesshoiOS/Kessho/Audio/AudioEngine.swift` around the reverb setup section
- dead send computations:
  - `KesshoiOS/Kessho/Audio/AudioEngine.swift` inside `applyParams()`
...
```sh
/bin/zsh -lc 'MODULE_CACHE=/tmp/swift-module-cache-audio && mkdir -p "$MODULE_CACHE" && /usr/bin/xcrun swiftc -typecheck -module-cache-path "$MODULE_CACHE" -sdk "$(xcrun --sdk iphoneos --show-sdk-path)" -target arm64-apple-ios17.0 $(find KesshoiOS/Kessho/Audio KesshoiOS/Kessho/Harmony KesshoiOS/Kessho/Services -name "*.swift" | sort) KesshoiOS/Kessho/State/SliderState.swift'
```
