...
- Generated `SetSourceOverride` event helpers remain wired for bounded sparse Pad, Lead, and Drum overrides, but source preset/body changes currently route through full snapshot/source-body reloads instead of live dirty diffs until a source-rebuild transaction exists.
...
- Runtime fallbacks are classified in `docs/kessho-product-runtime-fallback-classification.md` and enforced by `core:product:runtime-fallbacks`; missing `core-product` setter/update/reset/dice/control/getter methods now throw in development and increment/log diagnostics instead of silently disappearing in production, getter policy documentation is closed-list through `CORE_PRODUCT_GETTER_POLICIES`, reference-only fallback classification is not allowed in `core-product`, unknown telemetry/debug/analyser or legacy helper methods are forbidden instead of silently becoming optional UI paths, and the app/runtime product proxies no longer synthesize telemetry/analyser/debug/stem/preset-preview/diagnostics values before the selected engine is loaded.
...
- Product runtime switch state, runtime switch labels, and cross-runtime CPU summary storage now live in `src/ui/productRuntimeUi.ts`; `src/ui/audioEngineRuntimeUi.ts` remains only as a selected-runtime compatibility shim while broader App runtime decomposition continues.
