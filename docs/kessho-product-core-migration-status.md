...
- Product Default Gate v2 remains blocked by `native-default-deferred`, Product Core GitHub workflow/browser-readiness evidence, further web adapter decomposition, gate-quality classification, broader Lead/scene/full-arrangement parity, deterministic music/journey ownership closure, and final promotion review; local CPU p95/p99, bounded simulated-underrun, web worklet heap, decoded asset-memory, required unsupported-control gates, and FX/master depth checks now exist, but `core-product` must not become the web default while native release and remaining promotion blockers stand.
