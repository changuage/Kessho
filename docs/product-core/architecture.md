...
The remaining `applySequencerUiPatch` lane is an explicit temporary bridge for host-owned sequencer UI caches. It is blocked on Product-owned evolve config, sub-lane config, step override, pitch settings, and home-capture event or dirty-diff support; it must not expand into full snapshot reloads or one ProductEnginePort method per legacy setter.
