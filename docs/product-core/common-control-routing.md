...
| Sequencer evolve config edits | `productEngine.applySequencerUiPatch({ kind: 'drum-evolve-configs' / 'synth-evolve-configs' })` -> host sequencer UI patch bridge | Generated Product event batch or explicit product sequencer patch bridge | deferred-ticket | Explicit product sequencer patch lane, not full snapshot reload. TODO(product-core-control-routing-sequencer) tracks replacing cache-shaped payloads where Product events can update runtime and host caches atomically. |
| Sequencer sub-lane enabled edits | `productEngine.applySequencerUiPatch({ kind: '*-sub-lane-enabled' })` | Generated Product sub-lane config events or explicit product sequencer patch bridge | deferred-ticket | Explicit product sequencer patch lane. Event builders exist for sub-lane config, but host/UI cache reconciliation still uses the ticketed product patch bridge under TODO(product-core-control-routing-sequencer). |
| Sequencer pitch settings and step overrides | `productEngine.applySequencerUiPatch({ kind: 'synth-pitch-settings' / '*-step-overrides' })` | Generated Product step/value/config events or explicit product patch bridge | deferred-ticket | Explicit product sequencer patch lane. This remains the largest sequencer compatibility area, but it does not depend on full snapshot reloads and is ticketed under TODO(product-core-control-routing-sequencer). |
| Sequencer preset home snapshots and lane home capture | `productEngine.applySequencerUiPatch({ kind: 'preset-home-snapshots' / 'capture-*-lane-home' })` | Explicit product sequencer home/capture event or patch path | deferred-ticket | Host cache ownership still needs a product-shaped event contract; current path is an explicit product patch bridge ticketed under TODO(product-core-control-routing-sequencer). |
