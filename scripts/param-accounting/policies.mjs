...
export const productDeferredClassifications = [
  ...
  {
    id: 'sequencer-solo-ui-policy',
    owner: 'UI sequencer mute/solo state owner',
    allowWiredReferences: true,
    reason:
      'Sequencer solo is persisted host performance state; Product Core receives the resulting lane enable policy through existing generated lane-enabled events.',
...
export const EXPECTED_PARAM_REGISTRY_OMISSIONS = [
  ...
  {
    key: 'drumEuclid1Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting drumEuclid1Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'drumEuclid2Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting drumEuclid2Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'drumEuclid3Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting drumEuclid3Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'drumEuclid4Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting drumEuclid4Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'drumEuclid5Solo',
    reason: 'UI sequencer solo state persists the Scatter memory slot intent; committed Product playback remains represented by Product-owned drum lanes.',
  },
  {
    key: 'drumEuclid6Solo',
    reason: 'UI sequencer solo state persists the Scatter memory slot intent; committed Product playback remains represented by Product-owned drum lanes.',
  ...
  {
    key: 'synthEuclid1Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting synthEuclid1Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'synthEuclid2Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting synthEuclid2Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'synthEuclid3Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting synthEuclid3Enabled lane event instead of a separate solo param.',
  },
  {
    key: 'synthEuclid4Solo',
    reason: 'UI sequencer solo state persists the performance intent; Product Core receives the resulting synthEuclid4Enabled lane event instead of a separate solo param.',
  },
  {
