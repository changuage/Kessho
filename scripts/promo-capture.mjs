...
try {
  ...
  // A real user gesture is useful for WebAudio/autoplay policies.
  await page.mouse.click(960, 540).catch(() => {});
  await tryStartPlayback();
  await capture('01-playing-home');

  // The production app exposes top-level keyboard shortcuts; use those first,
  // then fall back to clicking visible tab labels.
  await openAdvancedTab('2', 'Synth', '02-synth', 4500);

  // Capture the sequencer faces exactly as presented in the web app.
  ...
  // Reactive Visualizer is the '=' top-level shortcut in production.
  ...
  // Return to synth for a final moving instrument shot.
