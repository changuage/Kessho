...
await runCheckWithReport({
  ...
  run: async (report) => {
    ...
    for (const token of [
      'this.sourcePresetEndpointBodyChanged(previousSource, nextSource)) return false',
      'canApplyCoreProductSourcePresetEndpointIdDiff(previousSource, nextSource)',
    ...
    for (const token of [
      ...
      assert(sourceOverrideDiffBody.includes(token), `sparse source override event helper is missing ${token}`);
    }
    ...
    assert(
      audioEngineParamSync.includes('SOURCE_PRESET_ENDPOINT_RESOLVED_COMMIT_KEYS') &&
        ...
        audioEngineParamSync.includes('Object.keys(patch).some(isSourceCoreResolvedCommitPatchKey)') &&
        audioEngineParamSync.includes("'drumKickPresetA'") &&
      ...
      'Product Core UI sync must resolve source preset endpoints/data and force full snapshots for source-body changes',
    );
    ...
    assert(
      padOverrideDiff.applied === false && padOverrideDiff.reason === 'pad-override-change',
      'Product Pad sparse override/body changes must full-snapshot so running sequencer voices rebuild audible source state',
    );
    ...
    assert(
      leadOverrideDiff.applied === false && leadOverrideDiff.reason === 'lead-override-change',
      'Product Lead sparse override/body changes must full-snapshot so running sequencer voices rebuild audible source state',
    );
    ...
    assert(
      drumOverrideDiff.applied === false && drumOverrideDiff.reason === 'drum-override-change',
      'Product Drum sparse override changes must full-snapshot so running sequencer voices rebuild audible source state',
    );
    ...
    assert(
      endpointDiff.applied === false && endpointDiff.reason === 'source-structure-change',
      'Pad/Lead source preset endpoint ID changes must full-snapshot so running sequencer voices rebuild audible source state',
    );
    ...
    assert(
      endpointAndOverrideDiff.applied === false && endpointAndOverrideDiff.reason === 'source-structure-change',
      'Pad/Lead source preset endpoint changes paired with sparse overrides must full-snapshot as source body changes',
    );
    ...
    assert(
      ...
      'preset morph changes after an endpoint+override full snapshot must remain dirty-diffed and must not become reset-prone',
    );
    ...
    assert(
      drumEndpointDiff.applied === false && drumEndpointDiff.reason === 'source-structure-change',
      'Product Drum preset endpoint changes must full-snapshot while preserving current drum voice morph in the rebuilt source state',
    );
    ...
    addEvidence(report, {
      ...
      details: {
        ...
        endpointReloadReason: endpointDiff.reason,
        drumOverrideReloadReason: drumOverrideDiff.reason,
        drumEndpointReloadReason: drumEndpointDiff.reason,
