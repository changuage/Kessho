...
final class DynamicsCharacterProcessor {
    ...
    private static func fillParams(from state: SliderState, sampleRate: Float, into params: inout [Float]) {
        ...
        let modeWow = depth * (0.08 + contribution.sineWow * 0.5)
        let modeFlutter = depth * (0.02 + contribution.flutterJitter * 0.12)
        let wowDamage = contribution.materialWear * 0.025 + contribution.aliasDamage * 0.018
        let flutterDamage = contribution.materialWear * 0.014 + contribution.aliasDamage * (0.018 + contribution.crossPatch * 0.074)
        let cyclicModeScale = modeActive ? 0.38 + degradeMix * 0.12 : 1
        let cyclicWow = clamp01(rawWow + modeWow * cyclicModeScale + wowDamage)
        let flutter = clamp01(rawFlutter + modeFlutter + flutterDamage)
        let cyclicFlutter = clamp01(rawFlutter + modeFlutter * (modeActive ? 0.55 + degradeMix * 0.1 : 1) + flutterDamage)
        ...
        let randomHoldRateHz: Double
        if mode == "shallowWater" {
            randomHoldRateHz = 0.08 + rate * 1.05 + depth * 0.18
        } else if mode == "abyssWater" {
            randomHoldRateHz = 0.045 + rate * 0.42 + envFollow * 0.04
        } else {
            randomHoldRateHz = 0.025 + rate * 0.14
        }

        let randomHoldLag: Double
        if mode == "shallowWater" {
            randomHoldLag = 0.18 + damp * 1.15
        } else if mode == "abyssWater" {
            randomHoldLag = 0.42 + damp * 1.8
        } else {
            randomHoldLag = 0.75 + damp * 2.1
        }
        ...
        let spreadBaseDelay = cleanFlavor > 0
            ? min(0.012, baseDelay + stereo * (0.0012 + depth * 0.0012) + drift * 0.0004)
        ...
        let randomFilterDepth = abyssFlavor > 0
            ? randomDrift * (80 + depth * 1050 + envFollow * 520) + modLp * 120
            : shallowFlavor > 0
                ? randomDrift * (20 + depth * 180) + modLp * 80
        ...
        set(.randomDriftDepth, randomDrift * (0.00014 + drift * 0.0012 + contribution.materialWear * 0.0007 + contribution.crossPatch * 0.0009))
        ...
        set(.mainDelayGain, 1 - stereo * (0.14 + shallowFlavor * 0.12))
        set(.spreadDelayGain, stereo * (cleanFlavor > 0 ? 0.05 + depth * 0.12 : 0.16 + depth * (0.4 + shallowFlavor * 0.18)))
        set(.wowFrequency, 0.03 + rate * 0.45 + drift * 0.18 + contribution.materialWear * 0.08)
        ...
        set(.wowDepth, (0.00016 + cyclicWow * 0.0062 + drift * 0.0022 + age * 0.0009) * (0.38 + depth * (0.78 + shallowFlavor * 0.18 + abyssFlavor * 0.06) + contribution.crossPatch * 0.34))
        set(.flutterDepth, (0.000006 + cyclicFlutter * 0.00072 + corrosion * 0.00012) * (0.24 + depth * (0.34 + shallowFlavor * 0.1) + contribution.crossPatch * 0.44))
    ...
    private static func characterDefaults(for mode: String) -> (
        ...
        switch mode {
        case "abyssWater":
            return (0.36, 0.06, 0.01, 0.82, 0.3, 0.33, 0.08, 0.33)
        case "shallowWater":
    ...
    private static func contributionMatrix(
        ...
        return (
            randomHold: clamp01(abyss * 0.55 + shallow * 0.52 + clean * 0.08 + materialWear * 0.08 + crossPatch * 0.28),
            smoothDrift: clamp01(abyss * 0.32 + shallow * 0.54 + clean * 0.12 + materialWear * 0.44 + crossPatch * 0.24),
            sineWow: clamp01(abyss * 0.1 + shallow * 0.42 + materialWear * 0.16 + crossPatch * 0.2),
