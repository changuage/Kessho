...
export function createCoreProductEvolvedStepValuePayload(options: {
  ...
  const entries = options.overrides.filter((entry) => entry.field === options.field).sort((left, right) => left.step - right.step);
...
export function syncCoreProductSequencerStepState(options: {
  ...
  for (let laneIndex = 0; laneIndex < laneCount; laneIndex += 1) {
    ...
    const activeStepValues = (values[laneIndex] ?? []).filter((stepValue) => coreProductStepValueFieldEnabled(options.synthSubLaneEnabled, options.drumSubLaneEnabled, options.sequencer, laneIndex, stepValue.field));
    const activeStepConfigs = (configs[laneIndex] ?? []).filter((config) => coreProductStepValueFieldEnabled(options.synthSubLaneEnabled, options.drumSubLaneEnabled, options.sequencer, laneIndex, config.field));
