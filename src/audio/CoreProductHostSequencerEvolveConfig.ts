...
export function diceFlagsForEvolveConfig(config: NormalizedSequencerEvolveConfig): number {
  ...
  let flags = 0;
  if (methods.rotateDrift || methods.hitDrift || methods.ghostNotes || methods.triggerToggle) flags |= CORE_PRODUCT_DICE_FLAGS.trigger;
  if (methods.probDrift) flags |= CORE_PRODUCT_DICE_FLAGS.probability;
  if (methods.ratchetSpray) flags |= CORE_PRODUCT_DICE_FLAGS.ratchet;
...
export function normalizeEvolveConfigs(configs: unknown): NormalizedSequencerEvolveConfig[] {
