/**
 * Looper-FX WASM Worklet Processor
 *
 * Thin AudioWorkletProcessor shell that loads kessho_looper.wasm and delegates
 * all DSP to the C++ engine.  Presents the **exact same postMessage interface**
 * as looper-fx.worklet.ts so that engine.ts can swap transparently.
 *
 * Message types received (matching looper-fx.worklet.ts):
 *   'params'          – full LooperParams object
...
interface KesshoWasm {
  ...
  // Lifecycle
  looper_init(sampleRate: number, bufferSeconds: number): number;
  looper_destroy(): void;
  // Buffer access
  looper_get_input_ptr(): number;
  looper_get_output_ptr(): number;
  // Processing
  looper_process_block(blockSize: number): void;
  // Global params
  looper_set_enabled(enabled: number): void;
  looper_set_freeze(frozen: number, withFeedback: number): void;
  looper_set_dry_wet(level: number): void;
  looper_set_feedback(amount: number, lpfHz: number): void;
  looper_set_scale(ptr: number, count: number): void;
  looper_set_buffer_size(seconds: number): void;
  // Per-voice
  looper_set_voice_mode(voice: number, enabled: number, mode: number): void;
  looper_set_voice_position(voice: number, slice: number, speed: number,
    reverse: number, pitch: number, writeFollow: number): void;
  looper_set_voice_grain(voice: number, density: number, grainSize: number,
    spray: number, grainOct: number, attack: number, decay: number): void;
  looper_set_voice_output(voice: number, gain: number, pan: number,
    blur: number, stereoSpread: number): void;
  looper_set_voice_lfo(voice: number, posRate: number, posDepth: number,
    panRate: number, reverseRate: number, recordRate: number): void;
  looper_set_voice_euclid_gated(voice: number, gated: number): void;
  looper_set_voice_euclid_muted(voice: number, muted: number): void;
  // Legacy
  looper_set_legacy_params(jitter: number, probability: number, pitchMode: number,
    pitchSpread: number, maxGrains: number, feedback: number): void;
  // Triggers
  looper_euclid_trigger(voice: number, velocity: number, sliceOverride: number,
    pitchOverride: number, hasPitch: number, reverseOverride: number, hasReverse: number): void;
  looper_set_random_sequence(ptr: number, count: number): void;
  // Queries
  looper_get_write_head(): number;
  looper_get_voice_positions(outPtr: number): void;
  looper_get_active_grain_count(): number;
  // Memory management
...
class LooperFXWasmProcessor extends AudioWorkletProcessor {
  ...
  private params: LooperParams | null = null;
  ...
  private async initWasm(wasmBinary: ArrayBuffer) {
    ...
    const result = exports.looper_init(sampleRate, bufferSeconds);
    if (result !== 0) {
      console.error('[LooperFX-WASM] Init failed:', result);
      return;
    }

    // Cache buffer pointers
    this.inputPtr = exports.looper_get_input_ptr();
    this.outputPtr = exports.looper_get_output_ptr();
    ...
    if (this.pendingRandomSequence) {
      ...
      this.wasm!.looper_set_random_sequence(ptr, seq.length);
      this.wasm!.free(ptr);
    }
  }
  ...
  private handleMessage(data: { type: string; [key: string]: unknown }) {
    switch (data.type) {
      ...
      case 'params':
        this.params = Object.assign(this.params || {} as LooperParams, data.params as Partial<LooperParams>);
      ...
      case 'euclidTrigger': {
        ...
        this.wasm.looper_euclid_trigger(voice, velocity, sliceOverride,
  ...
  /** Translate LooperParams → C API calls */
  private applyParams(p: LooperParams) {
    const w = this.wasm!;

    // Global
    w.looper_set_enabled(p.enabled ? 1 : 0);
    w.looper_set_freeze(p.freeze ? 1 : 0, p.freezeWithFeedback ? 1 : 0);
    w.looper_set_dry_wet(p.dryWet);
    w.looper_set_feedback(p.feedback, p.feedbackLPF);
    w.looper_set_buffer_size(p.bufferSeconds);
    ...
    if (p.scaleIntervals && p.scaleIntervals.length > 0) {
      ...
      w.looper_set_scale(this.scalePtr, count);
    } else {
      w.looper_set_scale(0, 0);
    }

    // Per-voice
    for (let v = 0; v < NUM_VOICES; v++) {
      w.looper_set_voice_mode(v,
        p.voiceEnabled[v] ? 1 : 0,
        MODE_MAP[p.voiceMode[v]] ?? 1);

      w.looper_set_voice_position(v,
        p.voiceSlice[v] || 0,
        p.voiceSpeed[v] ?? 1,
      ...
      w.looper_set_voice_output(v,
        p.voiceGain[v] ?? 0.5,
        p.voicePan[v] || 0,
        p.voiceBlur[v] || 0,
        p.voiceStereoSpread[v] ?? 0.5);

      w.looper_set_voice_lfo(v,
      ...
      w.looper_set_voice_euclid_gated(v, p.euclidGated[v] ? 1 : 0);
      w.looper_set_voice_euclid_muted(v, p.euclidMuted[v] ? 1 : 0);
    }

    // Legacy
    w.looper_set_legacy_params(
      p.legacyJitter,
      p.legacyProbability,
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][], _params: Record<string, Float32Array>) {
    ...
    // ── Process ──
    this.wasm.looper_process_block(blockSize);
    ...
    if (this.posReportCounter >= POS_REPORT_INTERVAL) {
      ...
      this.wasm.looper_get_voice_positions(this.positionsPtr);
      const posOffset = this.positionsPtr >> 2;
      this.port.postMessage({
        type: 'position',
        writeHead: this.wasm.looper_get_write_head(),
        voices: [
          freshHeap[posOffset],
...
registerProcessor('looper-fx-wasm', LooperFXWasmProcessor);
