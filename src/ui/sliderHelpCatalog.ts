...
const routingEntries: Record<string, SliderHelpEntry> = {
  ...
  routingMatrixDegradeColumn: entry(
...
const reverbEntries: Record<string, SliderHelpEntry> = {
  ...
  reverbBloom: entry('Sets tail direction through the reverb space.', 'Left pulls the wash into reverse swells. Center suspends the cloud. Right blooms the tail forward.', [rv('Shimmer & Effects', 'Bloom')]),
...
const dynamicsEntries: Record<string, SliderHelpEntry> = {
  tabDynamics: entry('Opens the Texture page.', 'Texture gathers Degrade movement, Dynamics Bus shaping, saturation, and final bus compression in one page.', [dn('Navigation', 'Texture Tab', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  dynamicsBusEnabled: entry('Enables the Dynamics Bus parent.', 'Dynamics Bus groups EQ 1, EQ 2, and Sidechain Compression so those terminal bus tools can be saved and recalled together.', [dn('Dynamics Bus', 'FX', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  dynamicsEqVisualizer: entry('Edits the EQ response curve.', 'Drag a band dot to change frequency and gain. Drag the smaller Q handle beside a band to narrow or widen that band.', [dn('Dynamics Bus', 'EQ Visualizer', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  dynamicsEq1Enabled: entry('Enables EQ 1 on the Dynamics Bus.', 'EQ 1 is the first editable three-band terminal EQ slot in the Dynamics Bus preset.', [dn('Dynamics Bus / EQ 1', 'FX', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  dynamicsEq2Enabled: entry('Enables EQ 2 on the Dynamics Bus.', 'EQ 2 is the second editable three-band terminal EQ slot in the Dynamics Bus preset.', [dn('Dynamics Bus / EQ 2', 'FX', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  dynamicsEq1InputGain: lowHigh('Sets EQ 1 input gain.', 'feed EQ 1 more quietly', 'push more level into EQ 1', [dn('Dynamics Bus / EQ 1', 'Input')]),
  dynamicsEq1OutputGain: lowHigh('Sets EQ 1 output gain.', 'trim level after EQ 1', 'raise level after EQ 1', [dn('Dynamics Bus / EQ 1', 'Output')]),
  dynamicsEq1LowFreq: lowHigh('Sets EQ 1 low band frequency.', 'move the low band lower', 'move the low band higher', [dn('Dynamics Bus / EQ 1', 'Low Freq')]),
  dynamicsEq1LowGain: lowHigh('Sets EQ 1 low band gain.', 'cut the low band', 'boost the low band', [dn('Dynamics Bus / EQ 1', 'Low Gain')]),
  dynamicsEq1LowQ: lowHigh('Sets EQ 1 low band Q.', 'make the low band wider', 'make the low band narrower', [dn('Dynamics Bus / EQ 1', 'Low Q')]),
  dynamicsEq1LowSlope: lowHigh('Sets EQ 1 low shelf slope.', 'make the shelf gentler', 'make the shelf steeper', [dn('Dynamics Bus / EQ 1', 'Low Slope')]),
  dynamicsEq1MidFreq: lowHigh('Sets EQ 1 mid band frequency.', 'move the mid bell lower', 'move the mid bell higher', [dn('Dynamics Bus / EQ 1', 'Mid Freq')]),
  dynamicsEq1MidGain: lowHigh('Sets EQ 1 mid band gain.', 'cut the mid band', 'boost the mid band', [dn('Dynamics Bus / EQ 1', 'Mid Gain')]),
  dynamicsEq1MidQ: lowHigh('Sets EQ 1 mid band Q.', 'make the mid bell wider', 'make the mid bell narrower', [dn('Dynamics Bus / EQ 1', 'Mid Q')]),
  dynamicsEq1HighFreq: lowHigh('Sets EQ 1 high band frequency.', 'move the high band lower', 'move the high band higher', [dn('Dynamics Bus / EQ 1', 'High Freq')]),
  dynamicsEq1HighGain: lowHigh('Sets EQ 1 high band gain.', 'cut the high band', 'boost the high band', [dn('Dynamics Bus / EQ 1', 'High Gain')]),
  dynamicsEq1HighQ: lowHigh('Sets EQ 1 high band Q.', 'make the high band wider', 'make the high band narrower', [dn('Dynamics Bus / EQ 1', 'High Q')]),
  dynamicsEq1HighSlope: lowHigh('Sets EQ 1 high shelf slope.', 'make the shelf gentler', 'make the shelf steeper', [dn('Dynamics Bus / EQ 1', 'High Slope')]),
  dynamicsEq2InputGain: lowHigh('Sets EQ 2 input gain.', 'feed EQ 2 more quietly', 'push more level into EQ 2', [dn('Dynamics Bus / EQ 2', 'Input')]),
  dynamicsEq2OutputGain: lowHigh('Sets EQ 2 output gain.', 'trim level after EQ 2', 'raise level after EQ 2', [dn('Dynamics Bus / EQ 2', 'Output')]),
  dynamicsEq2LowFreq: lowHigh('Sets EQ 2 low band frequency.', 'move the low band lower', 'move the low band higher', [dn('Dynamics Bus / EQ 2', 'Low Freq')]),
  dynamicsEq2LowGain: lowHigh('Sets EQ 2 low band gain.', 'cut the low band', 'boost the low band', [dn('Dynamics Bus / EQ 2', 'Low Gain')]),
  dynamicsEq2LowQ: lowHigh('Sets EQ 2 low band Q.', 'make the low band wider', 'make the low band narrower', [dn('Dynamics Bus / EQ 2', 'Low Q')]),
  dynamicsEq2LowSlope: lowHigh('Sets EQ 2 low shelf slope.', 'make the shelf gentler', 'make the shelf steeper', [dn('Dynamics Bus / EQ 2', 'Low Slope')]),
  dynamicsEq2MidFreq: lowHigh('Sets EQ 2 mid band frequency.', 'move the mid bell lower', 'move the mid bell higher', [dn('Dynamics Bus / EQ 2', 'Mid Freq')]),
  dynamicsEq2MidGain: lowHigh('Sets EQ 2 mid band gain.', 'cut the mid band', 'boost the mid band', [dn('Dynamics Bus / EQ 2', 'Mid Gain')]),
  dynamicsEq2MidQ: lowHigh('Sets EQ 2 mid band Q.', 'make the mid bell wider', 'make the mid bell narrower', [dn('Dynamics Bus / EQ 2', 'Mid Q')]),
  dynamicsEq2HighFreq: lowHigh('Sets EQ 2 high band frequency.', 'move the high band lower', 'move the high band higher', [dn('Dynamics Bus / EQ 2', 'High Freq')]),
  dynamicsEq2HighGain: lowHigh('Sets EQ 2 high band gain.', 'cut the high band', 'boost the high band', [dn('Dynamics Bus / EQ 2', 'High Gain')]),
  dynamicsEq2HighQ: lowHigh('Sets EQ 2 high band Q.', 'make the high band wider', 'make the high band narrower', [dn('Dynamics Bus / EQ 2', 'High Q')]),
  dynamicsEq2HighSlope: lowHigh('Sets EQ 2 high shelf slope.', 'make the shelf gentler', 'make the shelf steeper', [dn('Dynamics Bus / EQ 2', 'High Slope')]),
  sidechainEnabled: entry('Enables trigger-derived bus ducking.', 'Sidechain Compression listens to the selected drum voices and ducks the selected dry, delay, granular, and reverb target branches.', [dn('Dynamics Bus / Sidechain Compression', 'FX', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  sidechainVisualizer: entry('Shows the live sidechain shape.', 'The left side shows recent ducking events over time; the right side shows the editable detector curve plus attack, hold, and release timing handles.', [dn('Dynamics Bus / Sidechain Compression', 'Visualizer', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  ...
  endCompPeakBlend: lowHigh('Blends RMS and peak detection.', 'make the end-chain compressor follow average level more smoothly', 'make it react faster to short peaks and transient spikes', [dn('End Chain', 'Peak / RMS')]),
  endCompClarity: lowHigh('Adds post-compression clarity lift.', 'leave the compressed bus darker and flatter', 'restore more presence after compression without raising the whole bus', [dn('End Chain', 'Clarity')]),
  endCompTwoBandAmount: lowHigh('Blends the two-band compressor branch.', 'keep end-chain compression broadband', 'split low and high reduction more strongly in 2-Band mode', [dn('End Chain', '2-Band')]),
  endCompBandSplit: lowHigh('Sets the 2-band compressor crossover.', 'put more low-mid body in the high band', 'move the split upward so more warmth stays in the low band', [dn('End Chain', 'Band Split')]),
  driftEnabled: entry('Enables drift movement.', 'Drift can be bypassed from the section header without losing the selected movement mode or control values.', [dn('Drift', 'FX', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  driftVisualizer: entry('Shows the live drift shape.', 'The left side shows resolved motion depths — wow, flutter, drift, delay modulation, hold rate, stereo spread, and mix — with a live envelope meter and stereo field indicator. The right side shows the live filter response curve with high-pass, low-pass bias floor, LPG envelope opening, and resonance.', [dn('Drift', 'Visualizer', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  driftMix: lowHigh('Blends drift processing.', 'keep the clean bus forward', 'lean into the drift path', [dn('Drift', 'Mix')]),
  driftAge: lowHigh('Ages the drift path.', 'keep movement and loss subtle', 'add older, less stable behavior', [dn('Drift', 'Age')]),
  driftBias: lowHigh('Sets the base low-pass floor for the drift wet path.', 'keep the wet path darker and lower', 'start the wet path brighter before the LPG opens', [dn('Drift', 'Bias')]),
  driftLpgAmount: lowHigh('Sets how far the drift low-pass gate opens above its bias floor.', 'keep the LPG movement restrained', 'let the envelope open the filter much farther', [dn('Drift', 'LPG Open')]),
  driftDepth: lowHigh('Sets drift modulation depth.', 'make movement shallow', 'make warble and filtering move farther', [dn('Drift', 'Depth')]),
  driftRate: lowHigh('Sets drift modulation rate.', 'move slowly', 'move faster', [dn('Drift', 'Rate')]),
  driftDamp: lowHigh('Smooths the random drift CV.', 'let the random delay modulation jump more abruptly', 'slew the random delay modulation into slower, softer curves', [dn('Drift', 'Damp')]),
  driftEnvFollow: lowHigh('Sets how much input dynamics drive the low-pass gate.', 'keep the filter opening steadier', 'let transients open the watery lowpass path more strongly', [dn('Drift', 'Env Follow')]),
  driftWetHp: lowHigh('High-passes the drift wet path.', 'keep more low end in the modulated path', 'thin the wet path and clear more bass from the movement layer', [dn('Drift', 'Wet HP')]),
  driftStereo: lowHigh('Sets stereo movement spread.', 'keep the drift path centered', 'add dual-delay width and side-to-side drift', [dn('Drift', 'Stereo')]),
  driftResonance: lowHigh('Sets drift filter resonance.', 'keep filtering smooth', 'add more resonant color', [dn('Drift', 'Resonance')]),
  driftAntiComb: lowHigh('Protects Drift from comb filtering at mixed settings.', 'allow shorter modulation delay times with less guard spacing', 'raise the minimum delay guard when wet and dry are blended', [dn('Drift', 'Comb Protect')]),
  driftDiffusion: lowHigh('Adds Drift diffusion in the wet path.', 'keep modulation clearer and more direct', 'smooth short-delay motion with extra allpass diffusion and decorrelation', [dn('Drift', 'Diffusion')]),
  erosionEnabled: entry('Enables media erosion.', 'Erosion can be bypassed from the section header without losing wear, tone, saturation, or tape settings.', [dn('Erosion', 'FX', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  erosionMix: lowHigh('Blends the Degrade wet path.', 'keep the damaged path tucked away', 'bring the tape/lo-fi path forward', [dn('Degrade', 'Mix')]),
  erosionAge: lowHigh('Sets media wear.', 'keep the transport newer and cleaner', 'make pitch, bandwidth, noise, and corrosion feel older', [dn('Degrade', 'Wear')]),
  erosionGeneration: lowHigh('Adds copied-media loss.', 'keep the copy cleaner and closer to first generation', 'darken and smear the signal like repeated copies', [dn('Degrade', 'Generation')]),
  erosionAlias: lowHigh('Adds sample-rate and bit-depth damage.', 'keep the degraded path smoother', 'add stepped, aliased digital grit', [dn('Degrade', 'Alias')]),
  erosionWow: lowHigh('Sets slow pitch drift.', 'keep pitch steadier', 'add wider slow warble', [dn('Degrade', 'Wow')]),
  erosionFlutter: lowHigh('Sets fast pitch flutter.', 'keep pitch steadier', 'add faster tape-like flutter', [dn('Degrade', 'Flutter')]),
  erosionDrift: lowHigh('Sets long movement drift.', 'keep material centered', 'add slower wandering instability', [dn('Degrade', 'Drift')]),
  erosionWobbleSpeed: lowHigh('Sets tape wobble speed.', 'make wow wander more slowly', 'make tape bends move faster', [dn('Degrade', 'Wobble Speed')]),
  erosionNoise: lowHigh('Sets media noise.', 'keep the floor cleaner', 'add more hiss and grain', [dn('Degrade', 'Noise')]),
  degradeHp: lowHigh('Sets the shared dynamics high-pass.', 'keep more low body in Drift and Degrade', 'thin the low end more', [dn('Drift', 'HP'), dn('Degrade', 'HP')]),
  degradeLp: lowHigh('Sets the shared dynamics low-pass.', 'darken Drift and Degrade together', 'keep more top end', [dn('Drift', 'LP'), dn('Degrade', 'LP')]),
  erosionTone: lowHigh('Tilts Degrade tone.', 'favor a darker worn edge', 'favor brighter presence', [dn('Degrade', 'Tone')]),
  erosionSaturation: lowHigh('Sets Degrade-path clipping.', 'keep the damaged path cleaner', 'add more internal soft clipping', [dn('Degrade', 'Clip')]),
  erosionCorrosion: lowHigh('Sets degraded edge.', 'keep distortion smoother', 'add more broken alias-like edge', [dn('Degrade', 'Corrosion')]),
  erosionEventAmount: lowHigh('Sets media event depth.', 'keep dropouts and generation-loss dips rare and shallow', 'make media events pulse deeper and more visibly', [dn('Degrade', 'Events')]),
