...
const dynamicsEntries: Record<string, SliderHelpEntry> = {
  ...
  characterVisualizer: entry('Shows the live character shape.', 'The left side shows resolved motion depths — wow, flutter, drift, delay modulation, hold rate, stereo spread, and mix — with a live envelope meter and stereo field indicator. The right side shows the live filter response curve with high-pass, low-pass bias floor, LPG envelope opening, and resonance.', [dn('Character', 'Visualizer', 'single-only', [GLOBAL_SINGLE_NOTE])]),
  ...
  characterBias: lowHigh('Sets the base low-pass floor for the character wet path.', 'keep the wet path darker and lower', 'start the wet path brighter before the LPG opens', [dn('Character', 'Bias')]),
  characterLpgAmount: lowHigh('Sets how far the character low-pass gate opens above its bias floor.', 'keep the LPG movement restrained', 'let the envelope open the filter much farther', [dn('Character', 'LPG Open')]),
  ...
  characterDamp: lowHigh('Smooths the random character CV.', 'let the random delay modulation jump more abruptly', 'slew the random delay modulation into slower, softer curves', [dn('Character', 'Damp')]),
  characterEnvFollow: lowHigh('Sets how much input dynamics drive the low-pass gate.', 'keep the filter opening steadier', 'let transients open the watery lowpass path more strongly', [dn('Character', 'Env Follow')]),
  characterWetHp: lowHigh('High-passes the character wet path.', 'keep more low end in the modulated path', 'thin the wet path and clear more bass from the movement layer', [dn('Character', 'Wet HP')]),
  ...
  characterAntiComb: lowHigh('Protects Character from comb filtering at mixed settings.', 'allow shorter modulation delay times with less guard spacing', 'raise the minimum delay guard when wet and dry are blended', [dn('Character', 'Comb Protect')]),
  characterDiffusion: lowHigh('Adds Character diffusion in the wet path.', 'keep modulation clearer and more direct', 'smooth short-delay motion with extra allpass diffusion and decorrelation', [dn('Character', 'Diffusion')]),
  ...
  degradeHp: lowHigh('Sets the shared dynamics high-pass.', 'keep more low body in Character and Degrade', 'thin the low end more', [dn('Character', 'HP'), dn('Degrade', 'HP')]),
  degradeLp: lowHigh('Sets the shared dynamics low-pass.', 'darken Character and Degrade together', 'keep more top end', [dn('Character', 'LP'), dn('Degrade', 'LP')]),
  ...
  degradeEventAmount: lowHigh('Sets media event depth.', 'keep dropouts and generation-loss dips rare and shallow', 'make media events pulse deeper and more visibly', [dn('Degrade', 'Events')]),
  degradeProfileAmount: lowHigh('Blends the media profile EQ.', 'keep the degraded path closer to the raw tone controls', 'add more body bump and presence notch from copied-media profiling', [dn('Degrade', 'Profile')]),
  degradeDitherAmount: lowHigh('Adds dither before Degrade quantization.', 'keep alias damage harder and more stepped', 'soften quantized edges with low-level triangular dither', [dn('Degrade', 'Dither')]),
