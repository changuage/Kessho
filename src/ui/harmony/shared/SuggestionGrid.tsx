...
export const SuggestionGrid: React.FC<SuggestionGridProps> = ({ suggestions, onSelect, onPress, onRelease, onSave, disabled = false, axis }) => <div className="harmony-suggestion-grid" aria-label="Chord suggestions">{suggestions.slice(0, 8).map((suggestion, index) => {
  ...
  return <button key={suggestion.id} type="button" disabled={disabled} onClick={() => onSelect?.(suggestion)} onPointerDown={(event) => { event.currentTarget.setPointerCapture?.(event.pointerId); onPress?.(suggestion); }} onPointerUp={(event) => { if (event.currentTarget.hasPointerCapture?.(event.pointerId)) event.currentTarget.releasePointerCapture?.(event.pointerId); onRelease?.(suggestion); }} onPointerCancel={(event) => { if (event.currentTarget.hasPointerCapture?.(event.pointerId)) event.currentTarget.releasePointerCapture?.(event.pointerId); onRelease?.(suggestion); }} onKeyDown={(event) => { if (event.key.toUpperCase() === trigger && !event.repeat) { event.preventDefault(); if (event.shiftKey) onSave?.(suggestion); else onPress?.(suggestion); } }} onKeyUp={(event) => { if (event.key.toUpperCase() === trigger) onRelease?.(suggestion); }}><kbd>{trigger}</kbd><strong>{suggestion.label}</strong>{suggestion.category && <small>{suggestion.category}</small>}<RelativeChordDotMap notes={notes} axis={axis} /></button>;
})}</div>;
