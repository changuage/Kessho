...
import { analyzeOverviewBank, applyHarmonyOverviewAction, makeUniqueHarmonySlot, overviewFocusTarget, overviewRows, planOverviewEmptyUnusedSlot, planOverviewReplaceReferences, toggleHarmonyOverviewNote, updateHarmonyOverviewDuration, virtualizeOverviewRows, type HarmonyOverviewMode } from './harmonyOverviewModel';
...
export function HarmonyOverviewSurface({ progression, slots, sequence = null, pitchAxis, keyboardRoot = 60, mode = 'arrange', selectedIndex: selectedIndexProp = 0, readOnly = false, onModeChange, onProgressionChange, onSlotsChange, onOverviewStateChange, onPlay, onPlayStart, onPlayRelease, onLatch, onStop, onPrint, selectedSuggestionLabel, onSuggestionReplace, onSuggestionInsert, onSuggestionSave, suggestions }: HarmonyOverviewSurfaceProps) {
  ...
  return <section className="harmony-overview-surface" aria-label="Harmony combined overview" tabIndex={0} onKeyDown={handleKeyDown} onBlur={() => { setPlayingIndex(null); onPlayRelease?.(); }}>
    ...
    {mode === 'arrange' && <>
      <div className="harmony-overview-action-bar" aria-label="Arrange actions"><button type="button" onClick={() => applyAction('add')} disabled={readOnly || rows.length >= 64}>Add</button><button className="overview-secondary-action" type="button" onClick={() => applyAction('duplicate')} disabled={readOnly || rows.length >= 64}>Duplicate</button><button className="overview-secondary-action" type="button" onClick={makeUnique} disabled={readOnly || selectedRow?.slotId == null}>Make Unique</button><button type="button" onClick={() => applyAction('moveUp')} disabled={readOnly || selectedIndex <= 0}>Move up ↑</button><button type="button" onClick={() => applyAction('moveDown')} disabled={readOnly || selectedIndex >= rows.length - 1}>Move down ↓</button><button className="overview-secondary-action" type="button" onClick={() => applyAction('delete')} disabled={readOnly || rows.length <= 1}>Delete</button><button type="button" onClick={undo} disabled={readOnly || undoStack.length === 0}>Undo</button><button type="button" onClick={print} disabled={readOnly || !onPrint}>Print</button><button type="button" onClick={onLatch} disabled={readOnly}>Latch</button><button type="button" onClick={onStop} disabled={readOnly}>Stop</button></div><details className="harmony-overview-mobile-overflow"><summary>More actions</summary><div><button type="button" onClick={() => applyAction('duplicate')} disabled={readOnly || rows.length >= 64}>Duplicate</button><button type="button" onClick={makeUnique} disabled={readOnly || selectedRow?.slotId == null}>Make Unique</button><button type="button" onClick={() => applyAction('delete')} disabled={readOnly || rows.length <= 1}>Delete</button></div></details>
      <div className="harmony-overview-row-viewport" onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)}>{visibleWindow.totalHeight > 0 && <div style={{ height: visibleWindow.totalHeight, position: 'relative' }}>{visibleWindow.rows.map((row) => <button key={row.id} type="button" tabIndex={visibleFocusId === row.id ? 0 : -1} className={`harmony-overview-row${selectedIndex === row.index ? ' selected' : ''}${playingIndex === row.index ? ' playing' : ''}`} style={{ position: rows.length > 24 ? 'absolute' : 'relative', top: rows.length > 24 ? row.index * 76 : undefined }} onFocus={() => setFocusedRowId(row.id)} onClick={() => { setSelectedIndex(row.index); setFocusedRowId(row.id); }} onPointerDown={(event) => { event.currentTarget.setPointerCapture?.(event.pointerId); setFocusedRowId(row.id); setPlayingIndex(row.index); playStart(row.exactMidiNotes, row.slotId); }} onPointerUp={(event) => { if (event.currentTarget.hasPointerCapture?.(event.pointerId)) event.currentTarget.releasePointerCapture?.(event.pointerId); setPlayingIndex(null); onPlayRelease?.(); }} onPointerCancel={() => { setPlayingIndex(null); onPlayRelease?.(); }}><span className="harmony-overview-row-index">E{row.index + 1}</span><strong>{row.label}</strong><small>{row.source.type === 'auto' ? 'Auto' : `S${row.slotId! + 1}`} · {row.duration.unit} {row.duration.value}</small><RelativeChordDotMap notes={row.exactMidiNotes} axis={axis} /><span className="harmony-overview-row-play">{playingIndex === row.index ? 'Playing' : 'Play'}</span></button>)}</div>}</div>
