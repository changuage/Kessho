/**
 * LooperPage — Unified Granular Looper FX UI
 *
 * 4-voice looper-chopper-granular engine with:
...
const VOICE_KEYS: VoicePrefix[] = [1, 2, 3, 4].map(n => ({
  enabled: `looperV${n}Enabled` as keyof SliderState,
  mode: `looperV${n}Mode` as keyof SliderState,
  slice: `looperV${n}Slice` as keyof SliderState,
  speed: `looperV${n}Speed` as keyof SliderState,
  reverse: `looperV${n}Reverse` as keyof SliderState,
  pitch: `looperV${n}Pitch` as keyof SliderState,
  attack: `looperV${n}Attack` as keyof SliderState,
  decay: `looperV${n}Decay` as keyof SliderState,
  blur: `looperV${n}Blur` as keyof SliderState,
  grainOct: `looperV${n}GrainOct` as keyof SliderState,
  spray: `looperV${n}Spray` as keyof SliderState,
  density: `looperV${n}Density` as keyof SliderState,
  grainSize: `looperV${n}GrainSize` as keyof SliderState,
  pan: `looperV${n}Pan` as keyof SliderState,
  gain: `looperV${n}Gain` as keyof SliderState,
  posLFORate: `looperV${n}PosLFORate` as keyof SliderState,
  posLFODepth: `looperV${n}PosLFODepth` as keyof SliderState,
  panLFORate: `looperV${n}PanLFORate` as keyof SliderState,
  stereoSpread: `looperV${n}StereoSpread` as keyof SliderState,
  reverseLFORate: `looperV${n}ReverseLFORate` as keyof SliderState,
  writeFollow: `looperV${n}WriteFollow` as keyof SliderState,
}));

// Looper presets
const LOOPER_PRESETS: { id: string; name: string }[] = [
  { id: 'init', name: 'Init' },
...
export interface LooperPageProps {
  state: SliderState;
  isMobile: boolean;
...
const LooperPage: React.FC<LooperPageProps> = ({
  state,
  isMobile: _isMobile,
  expandedPanels,
  togglePanel,
  ...
  const activeSlices = useMemo(() => {
    ...
    state.looperV1Enabled, state.looperV1Slice,
    state.looperV2Enabled, state.looperV2Slice,
    state.looperV3Enabled, state.looperV3Slice,
    state.looperV4Enabled, state.looperV4Slice,
    triggerOverrides,
  ]);
  ...
  return (
    <div className="looper-root">
      <div className="looper-container">
        ...
        {/* ═══════════════ SOUND PANEL (left) ═══════════════ */}
        <div className="looper-sound-panel">

          {/* ── Global Controls Bar ── */}
          <div className="looper-global-bar">
            <span className="looper-title">⊞ Looper FX</span>

            <button
              className={`looper-enable-btn${state.looperEnabled ? ' on' : ''}`}
              onClick={() => onSelectChange('looperEnabled' as keyof SliderState, !state.looperEnabled)}
            >
              {state.looperEnabled ? 'ON' : 'OFF'}
            </button>

            <button
              className={`looper-freeze-btn${state.looperFreeze ? ' frozen' : ''}`}
              onClick={() => onSelectChange('looperFreeze' as keyof SliderState, !state.looperFreeze)}
            >
              {state.looperFreeze ? '❄ FROZEN' : '❄ Freeze'}
            </button>

            <select
              className="looper-preset-select"
              value={state.looperPreset}
              onChange={e => onSelectChange('looperPreset' as keyof SliderState, e.target.value)}
            >
              {LOOPER_PRESETS.map(p => (
          ...
          <div className="looper-global-sliders">
            <Slider
              ...
              <Slider
                label="Feedback"
                value={state.looperFeedback}
                paramKey={'looperFeedback' as keyof SliderState}
                onChange={onParamChange}
                {...sliderProps('looperFeedback' as keyof SliderState)}
              />
              <Slider
                label="FB LPF"
                value={state.looperFeedbackLPF}
                paramKey={'looperFeedbackLPF' as keyof SliderState}
                unit="Hz"
                onChange={onParamChange}
                {...sliderProps('looperFeedbackLPF' as keyof SliderState)}
              />
              <Slider
                label="Reverb Send"
          ...
          {/* ── Source Sends ── */}
          <div className="looper-source-sends">
            <span className="looper-sends-label">Input Sources</span>
            <div className="looper-sends-sliders">
              <Slider label="Pad 1" value={state.looperPad1Send} paramKey={'looperPad1Send' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperPad1Send' as keyof SliderState)} />
              <Slider label="Pad 2" value={state.looperPad2Send} paramKey={'looperPad2Send' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperPad2Send' as keyof SliderState)} />
              <Slider label="Lead 1" value={state.looperLead1Send} paramKey={'looperLead1Send' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperLead1Send' as keyof SliderState)} />
              <Slider label="Lead 2" value={state.looperLead2Send} paramKey={'looperLead2Send' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperLead2Send' as keyof SliderState)} />
              <Slider label="Drums" value={state.looperDrumSend} paramKey={'looperDrumSend' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDrumSend' as keyof SliderState)} />
              <Slider label="Waves" value={state.looperWavesSend} paramKey={'looperWavesSend' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperWavesSend' as keyof SliderState)} />
            </div>
          </div>

          {/* ── Buffer Visualization ── */}
          <div className="looper-buffer-viz">
            <div className="looper-buffer-slices">
              {Array.from({ length: NUM_SLICES }, (_, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`looper-slice${activeSlices.has(i) ? ' active' : ''}${triggeredSlices.has(i) ? ' triggered' : ''}`}
                    title={`Slice ${i + 1}${voicesOnSlice.length > 0 ? ` (V${voicesOnSlice.map(v => v + 1).join('+')})` : ''}`}
                  >
                    <span className="looper-slice-label">{i + 1}</span>
                    {/* Voice color dots for assigned voices */}
                    {voicesOnSlice.length > 0 && (
                      <div className="looper-slice-voice-dots">
                        {voicesOnSlice.map(v => (
                          <span
                            key={v}
                            className="looper-slice-voice-dot"
            ...
            <div
              className={`looper-write-head${state.looperFreeze ? ' frozen' : ''}`}
            ...
            {[0, 1, 2, 3].map(v => (
              state[VOICE_KEYS[v].enabled] ? (
                <div
                  key={v}
                  className="looper-buffer-voice-marker"
                  style={{
                    left: `${(voicePositions[v] || 0) * 100}%`,
                    background: VOICE_COLORS[v],
                  }}
                >
                  <span className="looper-voice-marker-label">V{v + 1}</span>
                </div>
              ) : null
            ))}
            {/* Time scale ticks — adapt to buffer duration */}
            <div className="looper-buffer-time-ticks">
              {(() => {
                const bs = (state.looperBufferSeconds as number) || 16;
                const ticks = bs <= 4 ? [0, 1, 2, 3, 4] : [0, 4, 8, 12, 16];
                return ticks.map(t => (
                  <span key={t} className="looper-time-tick" style={{ left: `${(t / bs) * 100}%` }}>
                    {t}s
                  </span>
                ));
              })()}
          ...
          {/* ── Macro Sliders ── */}
          <div className="looper-macro-section">
            <div
              className={`section-header${expandedPanels.has('looperMacros') ? '' : ' collapsed'}`}
              onClick={() => togglePanel('looperMacros')}
            >
              <span className="section-header-content">
                Macros
              </span>
            </div>
            <div className={`section-body${expandedPanels.has('looperMacros') ? '' : ' collapsed'}`}>
              <div className="looper-grid-4">
                <Slider
                  label="Texture"
                  value={state.looperMacroTexture ?? 0.3}
                  paramKey={'looperMacroTexture' as keyof SliderState}
                  onChange={onParamChange}
                  {...sliderProps('looperMacroTexture' as keyof SliderState)}
                />
                <Slider
                  label="Complexity"
                  value={state.looperMacroComplexity ?? 0.2}
                  paramKey={'looperMacroComplexity' as keyof SliderState}
                  onChange={onParamChange}
                  {...sliderProps('looperMacroComplexity' as keyof SliderState)}
                />
                <Slider
                  label="Darkness"
                  value={state.looperMacroDarkness ?? 0.3}
                  paramKey={'looperMacroDarkness' as keyof SliderState}
                  onChange={onParamChange}
                  {...sliderProps('looperMacroDarkness' as keyof SliderState)}
                />
                <Slider
                  label="Chaos"
                  value={state.looperMacroChaos ?? 0.1}
                  paramKey={'looperMacroChaos' as keyof SliderState}
                  onChange={onParamChange}
                  {...sliderProps('looperMacroChaos' as keyof SliderState)}
                />
              </div>
            </div>
          </div>
          ...
          <div className="looper-delay-section">
            <div
              className={`section-header${expandedPanels.has('looperDelay') ? '' : ' collapsed'}`}
              onClick={() => togglePanel('looperDelay')}
            >
              <span className="section-header-content">
                Multi-Tap Delay
                <button
                  className={`delay-toggle-btn${state.looperDelayEnabled ? ' on' : ''}`}
                  onClick={(e) => { e.stopPropagation(); onSelectChange('looperDelayEnabled' as keyof SliderState, !state.looperDelayEnabled); }}
                >
                  {state.looperDelayEnabled ? 'ON' : 'OFF'}
                </button>
                <span className="delay-bpm-info">@ {state.drumEuclidBaseBPM} BPM</span>
              </span>
            </div>
            <div className={`section-body${expandedPanels.has('looperDelay') ? '' : ' collapsed'}`}>
              {state.looperDelayEnabled && (
                <>
                  <div className="delay-note-row">
                    <div className="delay-note-col">
                      <label>Time</label>
                      <select
                        value={state.looperDelayTime as string}
                        onChange={e => onSelectChange('looperDelayTime' as keyof SliderState, e.target.value)}
                      >
                  ...
                  <Slider label="Activity" value={state.looperDelayActivity ?? 0.3} paramKey={'looperDelayActivity' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDelayActivity' as keyof SliderState)} />
                  <Slider label="Repeats" value={state.looperDelayRepeats ?? 0.3} paramKey={'looperDelayRepeats' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDelayRepeats' as keyof SliderState)} />
                  <Slider label="Filter" value={state.looperDelayFilter ?? 0.5} paramKey={'looperDelayFilter' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDelayFilter' as keyof SliderState)} />
                  <Slider label="Vibrato" value={state.looperDelayVibrato ?? 0} paramKey={'looperDelayVibrato' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDelayVibrato' as keyof SliderState)} />
                  <Slider label="Mix" value={state.looperDelayMix ?? 0.3} paramKey={'looperDelayMix' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDelayMix' as keyof SliderState)} />
                  <Slider label="Reverb Send" value={state.looperDelayReverbSend ?? 0.4} paramKey={'looperDelayReverbSend' as keyof SliderState} onChange={onParamChange} {...sliderProps('looperDelayReverbSend' as keyof SliderState)} />
                </>
              )}
            </div>
          </div>
          ...
          <div className="looper-voices">
            ...
            return (
              <div
                key={v}
                className={`looper-voice-card${isExpanded ? ' editing' : ''}${!isEnabled ? ' disabled' : ''}`}
              >
                {/* Header */}
                <div
                  className="looper-voice-header"
                  onClick={() => toggleVoice(v)}
                >
                  <div
                    className="looper-voice-dot"
                    style={{ background: isEnabled ? VOICE_COLORS[v] : '#555' }}
                  />
                  <span className="looper-voice-name">{VOICE_NAMES[v]}</span>
                  <span className="looper-voice-mode-badge">{mode}</span>
                  <span className="looper-voice-summary">{summary}</span>

                  <button
                    className={`looper-voice-enable-btn${isEnabled ? ' on' : ''}`}
                    onClick={e => {
                      e.stopPropagation();
                  ...
                  <button className={`looper-voice-expand-btn${isExpanded ? ' active' : ''}`}>
                    {isExpanded ? '▲' : '▼'}
                  </button>
                </div>
                ...
                {/* Body (expanded) */}
                {isExpanded && (
                  <div className="looper-voice-body">

                    {/* Mode Select */}
                    <div className="looper-mode-row">
                      {(['clean', 'granular', 'legacy'] as VoiceMode[]).map(m => (
                        <button
                          key={m}
                          className={`looper-mode-btn${mode === m ? ' active' : ''}`}
                    ...
                    {/* ── Slice & Playback ── */}
                    <div className="looper-section-label">Slice & Playback</div>
                    <div className="looper-grid-4">
                      <Slider
                        label="Slice"
                      ...
                      <div className="looper-pitch-wrap">
                        <Slider
                          label="Pitch"
                        ...
                        {pitchOv !== undefined && pitchOv !== 0 && (
                          <span className="looper-pitch-flash" style={{ color: VOICE_COLORS[v] }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <button
                          className={`looper-reverse-btn${effectiveReverse ? ' active' : ''}${reverseTriggered ? ' seq-triggered' : ''}`}
                    ...
                    {mode !== 'clean' && (
                      <>
                        <div className="looper-section-label">Grain</div>
                        <div className="looper-grid-4">
                          <Slider
                            label="Density"
                    ...
                    {/* ── Envelope & Texture ── */}
                    <div className="looper-section-label">Envelope & Texture</div>
                    <div className="looper-grid-4">
                      <Slider
                        label="Attack"
                    ...
                    {/* ── Panning & Stereo ── */}
                    <div className="looper-section-label">Pan & Stereo</div>
                    <div className="looper-grid-3">
                      <Slider
                        label="Pan"
                    ...
                    {/* ── Position LFO ── */}
                    <div className="looper-section-label">Position LFO</div>
                    <div className="looper-grid-2">
                      <Slider
                        label="Pos Rate"
                    ...
                    {/* ── Modulation ── */}
                    <div className="looper-section-label">Modulation</div>
                    <div className="looper-grid-2">
                      <Slider
                        label="Rev LFO"
                    ...
                    {mode === 'legacy' && v === 0 && (
                      <div className="looper-legacy-section">
                        <div className="looper-legacy-label">Legacy Granulator</div>
                        <div className="looper-grid-3">
                          <Slider
                            label="Jitter"
                            value={state.looperLegacyJitter}
                            paramKey={'looperLegacyJitter' as keyof SliderState}
                            unit="ms"
                            onChange={onParamChange}
                            {...sliderProps('looperLegacyJitter' as keyof SliderState)}
                          />
                          <Slider
                            label="Probability"
                            value={state.looperLegacyProbability}
                            paramKey={'looperLegacyProbability' as keyof SliderState}
                            onChange={onParamChange}
                            {...sliderProps('looperLegacyProbability' as keyof SliderState)}
                          />
                          <Slider
                            label="Max Grains"
                            value={state.looperLegacyMaxGrains}
                            paramKey={'looperLegacyMaxGrains' as keyof SliderState}
                            onChange={onParamChange}
                            {...sliderProps('looperLegacyMaxGrains' as keyof SliderState)}
                          />
                        </div>
                        <div className="looper-grid-2" style={{ marginTop: 4 }}>
                          <Slider
                            label="Pitch Spread"
                            value={state.looperLegacyPitchSpread}
                            paramKey={'looperLegacyPitchSpread' as keyof SliderState}
                            unit="st"
                            onChange={onParamChange}
                            {...sliderProps('looperLegacyPitchSpread' as keyof SliderState)}
                          />
                          <Slider
                            label="Legacy FB"
                            value={state.looperLegacyFeedback}
                            paramKey={'looperLegacyFeedback' as keyof SliderState}
                            onChange={onParamChange}
                            {...sliderProps('looperLegacyFeedback' as keyof SliderState)}
                          />
                        </div>
                        <div style={{ marginTop: 4 }}>
                          <select
                            ...
                            value={state.looperLegacyPitchMode}
                            onChange={e => onSelectChange(
                              'looperLegacyPitchMode' as keyof SliderState,
        ...
        </div>{/* end .looper-sound-panel */}

        {/* ═══════════════ SEQUENCER PANEL (right) ═══════════════ */}
        <div className="looper-sequencer-panel">
          {/* Transport */}
          <div className="seq-transport">
            <button
              className={`seq-play-btn${state.looperEuclidMasterEnabled ? ' playing' : ''}`}
              onClick={() => {
                const next = !state.looperEuclidMasterEnabled;
                if (next && !state.looperEnabled) {
                  onSelectChange('looperEnabled' as keyof SliderState, true);
                }
                onSelectChange('looperEuclidMasterEnabled' as keyof SliderState, next);
              }}
            >
              {state.looperEuclidMasterEnabled ? '■' : '▶'}
            </button>
            <DragNumber
              value={state.looperEuclidBaseBPM as number}
              min={40}
              max={300}
              label="BPM"
              onChange={(v) => onParamChange('looperEuclidBaseBPM' as keyof SliderState, v)}
              shapeByDrag
            />
            ...
            <div>
              ...
              <div className="seq-body" style={{ '--sc': activeSeq.color } as React.CSSProperties}>
                ...
                <div className="seq-sources">
                  <span className="looper-lane-voice-label" style={{ color: activeSeq.color }}>
              ...
              <SeqMiniOverview
                ...
                colors={LOOPER_SEQ_LANE_CONFIGS.map(c => c.color)}
                ...
                {seq.sequencerModels.map((seqModel, row) => (
                  ...
                  >
                    <div className="seq-ov-header" onClick={() => { seq.setActiveTab(row); seq.setViewMode('detail'); }}>
                      ...
                      <span className="looper-ov-voice" style={{ color: VOICE_COLORS[row] }}>→ V{row + 1}</span>
