...
constexpr uint32_t kMaxOrbitTriggerLines = 5u;
...
struct AnchorWalkerState {
  ...
  uint32_t auto_rate = 4u;
  uint32_t auto_feel = 0u;
  float swing = 0.0f;
  bool lead_mode = true;
  bool mw_to_velocity = false;
  bool pitch_wheel_walk = false;
  int32_t gesture_pattern[kMaxAnchorWalkerPatternSteps]{2, -1, 2, -1, 4, -1, 2, -1};
  uint32_t gesture_pattern_length = 8u;
  int32_t active_pad_delta = 1;
  uint32_t layer_preset = 2u;
  float spread_seconds = 0.035f;
  AnchorWalkerLayerState layers[kMaxAnchorWalkerLayers]{};
  uint32_t layer_count = 3u;
