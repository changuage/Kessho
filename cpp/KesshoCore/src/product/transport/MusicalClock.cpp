...
double ProductTransport::barPositionAt(double sample_rate, uint64_t sample) const {
  ...
  return bar_position_origin + beats / static_cast<double>(std::max(1u, beats_per_bar));
}

uint64_t ProductTransport::barIndexAt(double sample_rate, uint64_t sample) const {
  return static_cast<uint64_t>(std::max(0.0, std::floor(barPositionAt(sample_rate, sample))));
}
...
double ProductTransport::phrasePositionAt(double sample_rate, uint64_t sample) const {
  ...
  if (!std::isfinite(period) || period <= 0.0) return phrase_position_origin;
  return phrase_position_origin + static_cast<double>(elapsed) / period;
}

uint64_t ProductTransport::phraseIndexAt(double sample_rate, uint64_t sample) const {
  return static_cast<uint64_t>(std::max(0.0, std::floor(phrasePositionAt(sample_rate, sample))));
}
