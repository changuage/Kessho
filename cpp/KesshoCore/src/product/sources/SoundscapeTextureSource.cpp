...
void KesshoProductEngine::ensureSoundscapeTextureVoice(
  ...
  while (queued_count < kSoundscapeTextureMinimumQueuedSlices || runtime.next_start_frame < lookahead_frame) {
    ...
    const double requested_slice_duration = static_cast<double>(soundscapeTextureParam(
        source,
        texture_slot,
        kSoundscapeTextureParamSliceDuration,
        texture_slot == kSoundscapeTextureSlotFrogs ? 18.0f : 20.0f));
    const double max_natural_slice = std::max(1.5, asset_duration - 0.05);
    const double slice_duration = std::min(
        std::max(requested_slice_duration, 1.5),
        max_natural_slice);
    ...
    const bool asset_too_short_for_requested_slice =
        requested_slice_duration > max_natural_slice || max_offset <= 0.0001;
    runtime.last_fallback_reason = asset_too_short_for_requested_slice
        ? kSoundscapeTextureFallbackAssetTooShortForSlice
        : kSoundscapeTextureFallbackNone;
