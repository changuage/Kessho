...
ALTER TABLE public.preset_version_content_refs_v2
  ADD CONSTRAINT preset_version_content_refs_v2_type_check CHECK (content_type IN (
    ...
    'sequencerArrangement', 'mixRouting'
  )),
...
BEGIN
  ...
  IF function_definition NOT LIKE '%drumSubVoice%' THEN
    ...
    IF function_definition NOT LIKE '%drumSubVoice%' THEN
      RAISE EXCEPTION 'Could not extend kessho_save_preset_v2 content type allowlist';
    END IF;
