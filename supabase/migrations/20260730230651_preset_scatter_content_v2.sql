...
BEGIN
  ...
  IF function_definition NOT LIKE '%scatterConfig%' THEN
    function_definition := replace(
      function_definition,
      E'''parameterBehaviorMap''\n    )',
      E'''parameterBehaviorMap'',\n      ''scatterConfig''\n    )'
    );
    IF function_definition NOT LIKE '%scatterConfig%' THEN
      RAISE EXCEPTION 'Could not extend kessho_save_preset_v2 scatter content allowlist';
    END IF;
