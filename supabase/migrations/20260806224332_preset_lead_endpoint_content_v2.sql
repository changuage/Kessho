...
ALTER TABLE public.preset_version_content_refs_v2
  ADD CONSTRAINT preset_version_content_refs_v2_type_check CHECK (content_type IN (
    ...
    'granularVoice', 'granularSelection', 'padVoice', 'lead4opfmPatch', 'sampleVoice', 'dynamicsEq',
...
BEGIN
  ...
  IF function_definition NOT LIKE '%lead4opfmPatch%' THEN
    ...
    IF function_definition NOT LIKE '%lead4opfmPatch%' THEN
      RAISE EXCEPTION 'Could not extend kessho_save_preset_v2 Lead endpoint content allowlist';
    END IF;
