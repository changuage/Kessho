/**
 * Kessho Looper-FX Granular Engine — C API Header
 *
 * Full C++ port of looper-fx.worklet.ts for compilation to:
...
#ifndef KESSHO_LOOPER_H
#define KESSHO_LOOPER_H
...
/**
 * Initialize the looper engine. Must be called once before any other function.
...
/**
 * Destroy the looper engine and free all memory.
 */
 ...
 * JS writes input samples here before calling looper_process_block().
 ...
 * JS reads output samples from here after looper_process_block() returns.
 * Size: MAX_BLOCK_SIZE * 2 floats.
 */
...
/**
 * Enable or disable the looper. When disabled, input passes through unchanged.
 */
 ...
 * @param max_grains     Maximum grain count (0–128)
